    ...
    <div className="min-h-screen bg-cover bg-cneter flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl">
      ...
      <form onSubmit={onsubmitHandler} className="border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg">
        <h2 className="font-medium text-2xl flex justify-center items-center">
          {state}
          {isdatasubmited && <img onClick={()=>setisdatasubmited(false)} src={assets.arrow_icon} className="w-5 cursor-pointer
          "/>}
        ...
        <div className="flex flex-col gap-2">
          {state === "Sign up" ? (<p className="text-sm text-gray-600">
            Already have  an account? <span onClick={() => { setstate('Login'); setisdatasubmited(false)}} className="font-medium text-violet-500 cursor-pointer ">Login here</span>
          </p>) : (<p className="text-sm text-gray-600">
              Create an account <span onClick={()=>{setstate("Sign up")}} className="font-medium text-violet-500 cursor-pointer ">Click here</span>
          </p>)}
        </div>
      </form>
    </div>
